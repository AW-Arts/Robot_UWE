...
class InteractiveArm:
    ...
    def __init__(
        ...
        *,
        ...
        # NOTE: Avoid tight_layout() with custom axes-based widgets.
        # Responsive layout is handled by _apply_responsive_layout().
        ...
        # --------------------------------------------------------------
        # Responsive UI layout registries (Matplotlib-only).
        self._panel_axes_registry = []  # (ax, rel_left, rel_bottom, rel_w, rel_h)
        self._left_axes_registry = []   # (ax, rel_left, rel_bottom, rel_w, rel_h)
        self._left_sidebar_left = 0.02
        self._left_sidebar_bottom = 0.08
        self._left_sidebar_width = 0.14
        self._left_sidebar_height = 0.88
        self._mode_label_ax = None
        self._recording_indicator_ax = None
    ...
    def _on_canvas_resized(self, _event) -> None:
        # Custom UI uses many absolute-positioned axes; avoid tight_layout()
        # and recompute positions based on the new figure size.
    ...
    def _update_raw_angle_button_visual(self) -> None:
        ...
        if self._show_raw_angles:
            ...
            self._raw_angle_button.color = "#e2e8f0"
            self._raw_angle_button.hovercolor = "#f1f5f9"
    ...
    # ------------------------------------------------------------------
    # Responsive layout + UI polish (Matplotlib-only; no extra dependencies)

    def _left_axes(self, rel_left: float, rel_bottom: float, rel_width: float, rel_height: float):
        """Create an axes positioned relative to the left sidebar region."""
        ax = self.figure.add_axes(
            [
                self._left_sidebar_left + rel_left * self._left_sidebar_width,
                self._left_sidebar_bottom + rel_bottom * self._left_sidebar_height,
                rel_width * self._left_sidebar_width,
                rel_height * self._left_sidebar_height,
            ]
        )
        self._left_axes_registry.append((ax, rel_left, rel_bottom, rel_width, rel_height))
        return ax

    def _apply_responsive_layout(self) -> None:
        """Recompute layout for sidebar, IK viewport, and right-hand panel.

        Also makes the IK viewport ~20% narrower than the original layout to give the
        right-hand panels more breathing room.
        """
        try:
            fig_w, fig_h = self.figure.get_size_inches()
        except Exception:
            fig_w, fig_h = (14.5, 8.5)
        aspect = fig_w / max(fig_h, 1e-6)

        outer = 0.018
        gap = 0.014
        wide_layout = aspect >= 1.25

        if wide_layout:
            left_w = 0.155
            right_w = 0.46
            available = 1.0 - 2 * outer - left_w - right_w - 2 * gap
            if available < 0.28:
                right_w = max(0.36, right_w - (0.28 - available))

            self._left_sidebar_left = outer
            self._left_sidebar_bottom = outer
            self._left_sidebar_width = left_w
            self._left_sidebar_height = 1.0 - 2 * outer

            self._panel_left = 1.0 - outer - right_w
            self._panel_bottom = outer
            self._panel_width = right_w
            self._panel_height = 1.0 - 2 * outer

            main_left = self._left_sidebar_left + self._left_sidebar_width + gap
            main_right = self._panel_left - gap
            main_bottom = outer
            main_top = 1.0 - outer
        else:
            panel_h = 0.36
            ...
            self._panel_left = outer
            self._panel_bottom = outer
            self._panel_width = 1.0 - 2 * outer
            self._panel_height = panel_h

            main_bottom = self._panel_bottom + self._panel_height + gap
            main_top = 1.0 - outer

            self._left_sidebar_left = outer
            self._left_sidebar_bottom = main_bottom
            self._left_sidebar_width = left_w
            self._left_sidebar_height = max(main_top - main_bottom, 0.25)

            main_left = self._left_sidebar_left + self._left_sidebar_width + gap
            main_right = 1.0 - outer

        main_width = max(main_right - main_left, 0.25)
        main_height = max(main_top - main_bottom, 0.25)

        try:
            self.ax.set_position([main_left, main_bottom, main_width, main_height])
        except Exception:
            pass

        for ax, rel_left, rel_bottom, rel_w, rel_h in getattr(self, "_panel_axes_registry", []):
            ax.set_position(
                [
                    self._panel_left + rel_left * self._panel_width,
                    self._panel_bottom + rel_bottom * self._panel_height,
                    rel_w * self._panel_width,
                    rel_h * self._panel_height,
                ]
            )
        for ax, rel_left, rel_bottom, rel_w, rel_h in getattr(self, "_left_axes_registry", []):
            ax.set_position(
                [
                    self._left_sidebar_left + rel_left * self._left_sidebar_width,
                    self._left_sidebar_bottom + rel_bottom * self._left_sidebar_height,
                    rel_w * self._left_sidebar_width,
                    rel_h * self._left_sidebar_height,
                ]
            )

        if getattr(self, "_recording_indicator_ax", None) is not None:
            badge_w = 0.13 if wide_layout else 0.20
            badge_h = 0.05
            self._recording_indicator_ax.set_position(
                [1.0 - outer - badge_w, 1.0 - outer - badge_h, badge_w, badge_h]
            )
        ...
        for value in self.__dict__.values():
            if isinstance(value, Button):
                ...
                for item in value.values():
                    if isinstance(item, Button) and id(item) not in seen:
                        seen.add(id(item))
                        yield item
    ...
    def _refresh_button_fonts(self) -> None:
        """Shrink button labels when the available pixel area is tight."""
        try:
            fig_px = float(self.figure.bbox.width)
        except Exception:
            fig_px = 1200.0

        for button in self._iter_all_buttons():
            if button is None or getattr(button, "ax", None) is None:
                continue
            label = getattr(button, "label", None)
            if label is None:
                continue
            txt = label.get_text() or ""
            lines = txt.split("\n")
            longest = max((len(line) for line in lines), default=1)
            ax_pos = button.ax.get_position()
            width_px = max(ax_pos.width * fig_px - 10.0, 20.0)
            target = width_px / (0.56 * max(longest, 1))
            size = max(7.0, min(11.0, target))
            try:
    ...
    def _create_controls(self) -> None:
        ...
        background.set_facecolor("#ffffff")
        ...
        self._panel_interactive_widgets = {key: [] for _, key in menu_entries}
        self._panel_menu_buttons = {}
        self._active_panel = None

        menu_top = 1.0 - self._panel_margin * 0.55
        menu_bottom_edge = 1.0 - self._panel_menu_height + self._panel_margin * 0.35
        menu_h = max(menu_top - menu_bottom_edge, 0.01)

        rows = 2 if len(menu_entries) > 5 else 1
        row_gap = 0.02
        row_h = (menu_h - row_gap * (rows - 1)) / rows
        button_h = row_h * 0.84
        row_pad = (row_h - button_h) / 2

        available_width = 1.0 - 2 * self._panel_margin
        col_gap = self._panel_margin
        row_splits = [menu_entries] if rows == 1 else [menu_entries[:4], menu_entries[4:]]

        for row_index, entries in enumerate(row_splits):
            if not entries:
                continue
            cols = len(entries)
            button_w = (available_width - (cols - 1) * col_gap) / cols
            row_bottom = menu_bottom_edge + (rows - 1 - row_index) * (row_h + row_gap) + row_pad

            for col_index, (label, key) in enumerate(entries):
                left = self._panel_margin + col_index * (button_w + col_gap)

                display_label = label
                if button_w < 0.16:
                    if label == "Motor config":
                        display_label = "Motor\ncfg"
                    elif label == "Calibration tour":
                        display_label = "Cal\ntour"

                ax = self._panel_axes(left, row_bottom, button_w, button_h)
                button = Button(ax, display_label, hovercolor="#dbeafe")
                button.color = "#e2e8f0"
        ...
        # Shared state used by multiple panels.
        self.buttons = {}
        ...
        self._panel_widgets["calibration_tour"] = self._build_calibration_tour_panel()
    ...
    def _build_mode_toggle_strip(self) -> None:
        ...
        label_ax = self._left_axes(0.06, 0.93, 0.88, 0.06)
        self._mode_label_ax = label_ax
        label_ax.axis("off")
        label_ax.text(
            0.0,
            0.5,
        ...
        for index, (mode, label) in enumerate((("live", "Live"), ("teach", "Teach"))):
            left = 0.06 + index * (button_w + gap)
            ax = self._left_axes(left, toggle_bottom, button_w, toggle_ax_h)
            button = Button(ax, label, hovercolor="#dbeafe")
            button.color = "#e2e8f0"
            button.hovercolor = "#dbeafe"
            button.ax.set_facecolor(button.color)
    ...
    def _build_recording_indicator(self) -> None:
        ...
        self._recording_indicator_ax = indicator_ax
        indicator_ax.axis("off")
        indicator_ax.set_facecolor("#ffffff")
    ...
    def _panel_axes(
        ...
        self._panel_axes_registry.append((ax, rel_left, rel_bottom, rel_width, rel_height))
        return ax
    ...
    def _set_active_panel(self, panel: str) -> None:
        ...
        for name, button in self._panel_menu_buttons.items():
            if name == panel:
                button.color = "#93c5fd"
                button.hovercolor = "#93c5fd"
            else:
                button.color = "#e2e8f0"
                button.hovercolor = "#f1f5f9"
            button.ax.set_facecolor(button.color)
    ...
    def _update_mode_controls(self) -> None:
        def _style_button(button: Button | None, active: bool, *, danger: bool = False) -> None:
            if button is None:
                return
            if active:
                button.color = "#93c5fd" if not danger else "#ffdec3"
    ...
    def _build_movement_panel(self) -> list:
        ...
        home_bottom = self._panel_margin
        slider_bottom = home_bottom + home_h + gap
        ...
        self._home_button = Button(home_ax, "Home", hovercolor="#dbeafe")
        self._home_button.color = "#e2e8f0"
        self._home_button.hovercolor = "#dbeafe"
        self._home_button.ax.set_facecolor(self._home_button.color)
        ...
        pad_w = max(pad_right - pad_left, 0.01)
        pad_h = max(pad_top - pad_bottom, 0.01)

        cell_gap = 0.02
        max_cell_w = (pad_w - 2 * cell_gap) / 3.0
        max_cell_h = (pad_h - 2 * cell_gap) / 3.0
        cell = max(0.06, min(max_cell_w, max_cell_h, 0.23))

        grid_w = 3 * cell + 2 * cell_gap
        grid_h = 3 * cell + 2 * cell_gap
        grid_left = pad_left + (pad_w - grid_w) / 2.0
        grid_bottom = pad_bottom + (pad_h - grid_h) / 2.0

        def _cell_axes(col: int, row: int):
            x = grid_left + col * (cell + cell_gap)
            y = grid_bottom + row * (cell + cell_gap)
            return self._panel_axes(x, y, cell, cell)

        def _make_button(ax, label, delta):
            btn = Button(ax, label, hovercolor="#dbeafe")
            btn.color = "#e2e8f0"
            btn.hovercolor = "#dbeafe"
            btn.ax.set_facecolor(btn.color)
            btn.on_clicked(self._make_move_callback(delta))
            self._panel_interactive_widgets[panel_key].append(btn)
            return btn
        ...
        for name, (col, row, label, delta) in button_defs.items():
            ax_btn = _cell_axes(col, row)
            self.buttons[name] = _make_button(ax_btn, label, delta)
            axes.append(ax_btn)
    ...
    def _update_calibration_button_visual(self) -> None:
        ...
        if self._calibration_active:
            ...
            self._calibration_button.color = "#e2e8f0"
            self._calibration_button.hovercolor = "#f1f5f9"
            self._calibration_button.label.set_text("Calibrate")
        ...
        if self._set_vertical_button is not None:
            ...
            if self._calibration_active:
                ...
                self._set_vertical_button.color = "#e2e8f0"
                self._set_vertical_button.hovercolor = "#f1f5f9"
